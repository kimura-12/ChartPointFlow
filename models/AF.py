...
class Flow(nn.Module):
    ...
    def __init__(self, context_dim, h_dims, y_dim, nonlinearity):
    ...
    def forward(self, x_in,  z, label, reverse=False):
        if reverse:
            x_a = torch.cat((x_in[:,:,0:1], label), dim=2)
            ...
            x_b = torch.cat((x_in[:,:,0:2], label), dim=2)
...
class AF(torch.nn.Module):
    def __init__(self, n_flows=8, z_dim=128, h_dims=(256, 256, 256), y_dim=12, nonlinearity="tanh"):
        ...
        for k in range(n_flows):
            ...
            self.WN.append(Flow(z_dim, h_dims, y_dim, nonlinearity))

    def set_initialized(self, switch):
        for actnorm in self.actnorm:
            actnorm.is_initialized=switch

    def forward(self, x,  z, label, reverse=False, sequence=False):
        if reverse:
            return self.reverse(x,  z, label)
        if sequence:
            return self.reverse_each_flow(x,  z, label)
        ...
        for k in range(self.n_flows):
            ...
            x_in = x[:,:,:-1]
            log_s, b = self.WN[k](x_in,  z, label)
    ...
    def reverse(self, z,  c, label):
        for k in reversed(range(self.n_flows)):
            z = self.WN[k](z,  c, label, reverse=True)
            z = self.convinv[k](z, reverse=True)
            z = self.actnorm[k](z, reverse=True)

        return z

    def reverse_each_flow(self, z,  c, label):
        each_z = []
        each_z.append(z)
        for k in reversed(range(self.n_flows)):
            z = self.WN[k](z,  c, label, reverse=True)
